...
export default function WorkingHours() {
  ...
  const updateBreak = (breakId, patch) => {
    setDraft((prev) => ({
      ...prev,
      breaks: prev.breaks.map((item) => (item.id === breakId ? { ...item, ...patch } : item)),
    }));
  };
  ...
  const updateOverrideSlot = (overrideId, slotId, patch) => {
    updateOverride(overrideId, (item) => ({
      ...item,
      slots: item.slots.map((slot) => (slot.id === slotId ? { ...slot, ...patch } : slot)),
    }));
  };
      ...
      {!validation.valid && (
        <div className="border border-warning/40 bg-warning-light rounded-md p-4">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-sm font-semibold text-text-primary">Resolve these issues to save</p>
                  ...
                  {activeSection === 'Weekly Hours' && (
                    <div className="space-y-4">
                      <div className="flex flex-wrap items-center gap-2 p-3 bg-surface-secondary border border-border rounded-md">
                        <span className="text-xs font-medium text-text-secondary">Copy pattern</span>
                        <select
                          value={copyFrom}
                      ...
                      {draft.weekly.map((daySchedule) => (
                        <div key={daySchedule.day} className="border border-border rounded-md p-4">
                          <div className="flex items-center justify-between gap-4">
                            <div>
                              <p className="text-sm font-semibold text-text-primary">{daySchedule.day}</p>
                              <p className="text-xs text-text-muted">
                                {daySchedule.enabled ? `${daySchedule.slots.length} slot(s)` : 'Closed'}
                              </p>
                            </div>
                              ...
                              {daySchedule.slots.map((slot) => (
                                <div key={slot.id} className="flex flex-wrap items-center gap-2">
                                  ...
                                  <input
                                    ...
                                    value={slot.end}
                                    onChange={(e) =>
                                      updateDaySlot(daySchedule.day, slot.id, { end: e.target.value })
                                    }
                                    className="h-9 px-2 text-sm border border-border rounded-md bg-surface"
                                  />
                                  <button
                                    onClick={() => removeDaySlot(daySchedule.day, slot.id)}
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-text-secondary">
                          Define recurring pauses (e.g., lunch) inside open windows.
                        </p>
                        <button
                          onClick={addBreak}
                          className="h-8 px-3 text-xs border border-border rounded-md hover:bg-surface-secondary"
                      ...
                      {draft.breaks.map((item) => (
                        <div key={item.id} className="grid grid-cols-1 md:grid-cols-[140px_1fr_120px_120px_40px] gap-2 border border-border rounded-md p-3">
                          <select
                            value={item.day}
                            onChange={(e) => updateBreak(item.id, { day: e.target.value })}
                            className="h-9 px-2 text-sm border border-border rounded-md bg-surface"
                          >
                            {getDayOrder().map((day) => (
                              <option key={day} value={day}>
                          ...
                          <input
                            value={item.label}
                            onChange={(e) => updateBreak(item.id, { label: e.target.value })}
                        ...
                        <div key={item.id} className="border border-border rounded-md p-4 space-y-3">
                          <div className="grid grid-cols-1 md:grid-cols-[170px_170px_1fr_40px] gap-2">
                            <input
                              ...
                              value={item.date}
                              onChange={(e) =>
                                updateOverride(item.id, (curr) => ({ ...curr, date: e.target.value }))
                              }
                              className="h-9 px-2 text-sm border border-border rounded-md bg-surface"
                            />
                            <select
                              ...
                              onChange={(e) =>
                                updateOverride(item.id, (curr) => ({
                                  ...curr,
                                  type: e.target.value,
                                  slots: e.target.value === 'custom' ? curr.slots : [],
                                }))
                              }
                              className="h-9 px-2 text-sm border border-border rounded-md bg-surface"
                            >
                            ...
                            <input
                              value={item.note}
                              onChange={(e) =>
                                updateOverride(item.id, (curr) => ({ ...curr, note: e.target.value }))
                              }
                              ...
                              {item.slots.map((slot) => (
                                <div key={slot.id} className="flex flex-wrap items-center gap-2">
                                  ...
                                  <input
                                    ...
                                    value={slot.end}
                                    onChange={(e) =>
                                      updateOverrideSlot(item.id, slot.id, { end: e.target.value })
                                    }
                                    className="h-9 px-2 text-sm border border-border rounded-md bg-surface"
                                  />
                                  <button
                                    onClick={() => removeOverrideSlot(item.id, slot.id)}
                                    className="h-9 w-9 inline-flex items-center justify-center border border-border rounded-md text-text-muted hover:text-danger hover:border-danger"
                  ...
                  {activeSection === 'Summary' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="border border-border rounded-md p-4 bg-surface-secondary">
                        ...
                        <p className="text-sm font-semibold text-text-primary mt-1">{draft.timezone}</p>
                      </div>
                      <div className="border border-border rounded-md p-4 bg-surface-secondary">
        ...
        <aside className="space-y-4">
          <div className="bg-surface border border-border rounded-md p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold text-text-primary">Patient Visibility Preview</h3>
            </div>

            <label className="text-xs text-text-muted">Simulate date & time</label>
            <input
              type="datetime-local"
              value={previewAt}
            ...
            >
              ...
              <p className="text-sm font-semibold text-text-primary mt-0.5">
                {previewState.isOpen ? 'Open now' : reasonLabel(previewState.reason)}
              </p>
              ...
              {!previewState.isOpen && previewState.nextOpenAt && (
                <p className="text-xs text-text-secondary mt-1">
                  Next opens {previewState.nextOpenAt.dayName} at {formatClock(previewState.nextOpenAt.time)}
                </p>
              )}
            </div>
            ...
            <div className="text-xs text-text-muted space-y-1">
              <p>
                <span className="text-text-secondary">Timezone:</span> {previewState.timezone}
              </p>
              <p>
                <span className="text-text-secondary">Coverage:</span> {coverage.weeklyOpenLabel} weekly
              </p>
              <p>
                <span className="text-text-secondary">Open days:</span> {coverage.openDays} / 7
              </p>
            </div>
          </div>
        </aside>
