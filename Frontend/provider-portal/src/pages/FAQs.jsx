...
import {
  ...
  Sparkles, HelpCircle, Loader2, X, ChevronDown, ChevronUp,
} from 'lucide-react';
import {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 mb-1">
            <span className="text-label uppercase text-text-muted flex-shrink-0">Q</span>
            <p className="text-sm font-semibold text-text-primary leading-snug">{faq.question}</p>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-label uppercase text-text-muted flex-shrink-0">A</span>
            <p className={cn('text-sm text-text-secondary leading-relaxed', !expanded && 'line-clamp-2')}>
              {faq.answer}
            </p>
          </div>
          ...
          {/* Hit count */}
          <p className="text-[11px] text-text-muted mt-2">Asked {faq.hits} times</p>
        </div>
        ...
        {showConfirm && (
          ...
          >
            <p className="text-sm text-text-primary mb-2">
              Delete this FAQ? <span className="text-text-muted">This cannot be undone.</span>
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => { onDelete(faq.id); setShowConfirm(false); }}
                disabled={isDeleting}
...
function FaqPanel({ editing, onClose }) {
  const { addFaq, updateFaq } = useClinicStore();
  ...
  const { register, handleSubmit, watch, setValue, reset, formState: { errors, isDirty } } = useForm({
    ...
    defaultValues: editing ? { question: editing.question, answer: editing.answer } : {},
  });
  ...
  const onSubmit = async (data) => {
    if (editing) {
      ...
      addFaq(data);
      toast.success('FAQ added! Your AI will use this in the next conversation.');
    }
    onClose();
  };
  ...
  const handleAiAssist = async () => {
    ...
    await sleep(1200);
    setValue('answer', `Based on the question, here is a helpful answer about "${question}". Patients frequently ask this, so we recommend being clear and concise. You can edit this before saving.`);
    setAiLoading(false);
  };
  ...
  return (
    <div className="bg-surface border border-border rounded-md p-5 h-fit sticky top-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-h4 text-text-primary">{editing ? 'Edit FAQ' : 'Add FAQ'}</h3>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-xs font-medium text-text-secondary">Question</label>
            <span className="text-[11px] text-text-muted">{question.length}/200</span>
          </div>
          <textarea
            {...register('question')}
            rows={3}
          ...
          />
          {errors.question && <p className="mt-1 text-xs text-danger">{errors.question.message}</p>}
        </div>

        {/* AI Assist toggle */}
        <div className="flex items-center justify-between p-3 bg-surface-secondary rounded-md border border-border">
          <div>
            <p className="text-xs font-medium text-text-primary">Auto-generate answer</p>
            <p className="text-[11px] text-text-muted">AI suggests an answer from your question</p>
          </div>
        ...
        {aiEnabled && (
          ...
          >
            {aiLoading ? <><Loader2 size={14} className="animate-spin" />Generating...</> : <><Sparkles size={14} />Generate Answer</>}
          </button>
        )}
        {aiEnabled && answer && (
          <p className="text-[11px] text-warning">Review AI-generated answer before saving.</p>
        )}

        {/* Answer */}
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-xs font-medium text-text-secondary">Answer</label>
            <span className="text-[11px] text-text-muted">{answer.length}/500</span>
          </div>
          {aiLoading ? (
          ...
          {errors.answer && <p className="mt-1 text-xs text-danger">{errors.answer.message}</p>}
        </div>
...
export default function FAQs() {
  const { faqs, deleteFaq, reorderFaqs } = useClinicStore();
  ...
  return (
    <div className="max-w-[1400px]">
      ...
      {/* ── Main Layout ─────────────────────────────────────────── */}
      <div className={cn('flex gap-5', panelOpen ? 'grid grid-cols-[1fr_360px]' : '')}>
        {/* List */}
        <div className="flex-1 min-w-0">
          {filtered.length === 0 ? (
            <EmptyState
              ...
              description={search ? 'Try a different search term.' : 'Add your first FAQ to help your AI give better answers.'}
              action={!search ? { label: 'Add Your First FAQ', onClick: () => setPanelOpen(true) } : undefined}
            />
          ) : (
            <DndContext
              sensors={sensors}
