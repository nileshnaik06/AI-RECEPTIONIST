...
import { Eye, EyeOff, RefreshCw, Plus, Trash2, Loader2, Shield, Info } from 'lucide-react';
...
export default function ApiSecurity() {
  ...
  const { allowedOrigins, addOrigin, removeOrigin } = useClinicStore();
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-sm text-text-muted mt-0.5">Manage your API key, access control, and account security.</p>
      </div>
      ...
      <section className="bg-surface border border-border rounded-md p-5">
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            ...
            <div key={label} className="bg-surface-secondary rounded-md p-3 text-center border border-border">
        ...
        <button
          ...
          className="flex items-center gap-2 text-sm text-danger hover:underline transition-colors"
        >
          <RefreshCw size={15} />
          Regenerate API Key
        </button>
      ...
      <section className="bg-surface border border-border rounded-md p-5">
        ...
        <div className="flex items-start gap-2 mb-4 p-3 bg-primary-light rounded-md border border-primary/20">
          ...
          <p className="text-xs text-primary">
            Only these domains can use your widget. Leave empty to allow all origins.
          </p>
        </div>

        <div className="space-y-2 mb-3">
          {allowedOrigins.map((domain) => (
            <div key={domain} className="flex items-center justify-between px-3 py-2.5 bg-surface-secondary border border-border rounded-md">
          ...
          {allowedOrigins.length === 0 && (
            <p className="text-sm text-text-muted py-2">All origins allowed (no restrictions).</p>
          )}
        </div>
      ...
      <section className="bg-surface border border-border rounded-md p-5">
        ...
        <form onSubmit={pwForm.handleSubmit(handleChangePassword)} noValidate className="space-y-4">
          {[
            ...
            <div key={name}>
              ...
              <input
                ...
                className={cn(
                  ...
                  pwForm.formState.errors[name] ? 'border-danger' : 'border-border'
                )}
              />
              {pwForm.formState.errors[name] && (
                <p className="mt-1 text-xs text-danger">{pwForm.formState.errors[name].message}</p>
              )}
            </div>
          ))}
          <div className="flex justify-end">
            <button type="submit" disabled={pwForm.formState.isSubmitting}
              ...
              {pwForm.formState.isSubmitting ? <><Loader2 size={14} className="animate-spin" />Updating...</> : 'Update Password'}
            </button>
          </div>
        </form>
      </section>
            ...
            <tbody>
              {MOCK_API_LOGS.map((log) => (
                <tr key={log.id}>
                  ...
                  <td><span className="badge bg-surface-secondary text-text-secondary">{log.method}</span></td>
                  <td><span className={cn('text-sm font-semibold', statusColor(log.status))}>{log.status}</span></td>
